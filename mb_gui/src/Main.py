...
Project_Slide = GuiFunctions.Slide()
...
def loading_slide_operation():
    ...
    read_slide_output = Project_Slide.funcReadSlide(slide_fullpath, prepath)
...
def section_detection_operation():
    ...
    # ToDo: technical Dept
    try:
        brainboundcoords, tissuemask_fullpath = Project_Slide.funcSectionDetection()
...
def section_select_operation():
    ...
    try:
        ...
        blob_detection_file_name, tissue_lm_detection_filename = Project_Slide.section.get_section_images(brnum, brainboundcoords)
    except Exception as e:
        print(e)
...
def blob_flip_operation():
    ...
    blob_detection_file_name, tissue_lm_detection_filename = Project_Slide.section.section_flip_operation()
...
def blob_detection_operation():
    ...
    if MainWindow.step_allowed_to_change(2):
        ...
        RED_COUNT, GREEN_COUNT,CO_COUNT,_,blobs_log_r,blobs_log_g,colocalized_blobs = Project_Slide.section.funcBlobDetection(brnum, blobs_parameters)
...
def landmark_auto_detect_operation(atlas_address=None):
    ...
    tissue_auto_landmarks = Project_Slide.section.funcLandmarkDetection(tissue_landmark_detection_file_name, 400)	
    atlas_auto_landmarks = Project_Slide.section.funcLandmarkDetection(atlas_landmark_detection_file_name, 500)	
...
def registration_accept():
    ...
    Uses the funcAMBIAregister / outputs the registered Project_Slide.section or atlas
    '''
    ...
    if len(img1coords)==len(img2coords):
        if atlasnum < 22:
            ...
            overlayed_registered_imgs_path, reg_code = func_ambia_registration(sectionfolder, source_img_path, target_img_path, img1coords, img2coords, Ardent_reg_done, atlas_img_path, Project_Slide.section, auto_registration_state)
...
def registration_preview_accept():
    ...
    Project_Slide.section.calculate_fp_fn_blobs(red_blobs_modified, green_blobs_modified)
    ...
    if len(red_blobs_modified)>0:
        red_converted_coords = func_convert_coords(reg_code, red_blobs_modified, 'red',Project_Slide.section)
    if len(green_blobs_modified)>0:
        green_converted_coords = func_convert_coords(reg_code, green_blobs_modified, 'green',Project_Slide.section)
    if len(colocalized_blobs_coords)>0:
        colocalized_converted_coords = func_convert_coords(reg_code, colocalized_blobs_coords, 'coloc',Project_Slide.section)
    GuiFunctions.save_to_saved_data_pickle(blobs_coords_orig, 'blobs_coords_orig', Project_Slide.section)
    #Project_Slide.section.save_to_pkl("blobs_coords_orig.pkl", blobs_coords_orig)
    ...
    f = open(os.path.join(mappingimgpath,  "reportfile.txt"), 'r+')
    message = f.read()
    MainWindow.set_report_text(message)
    MainWindow.finish_step(4)
    MainWindow.go_to_next_step()
    GuiFunctions.save_the_pickle(Project_Slide.section)
