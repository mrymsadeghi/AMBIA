...
def func_delauney_registration(section_savepath, source_lms, target_lms, source_img_path, target_img_path, reg_code):
    ...
    target_landmark_points = calculate_n_add_corner_points(target_gray, target_landmark_points, reg_code)
...
def func_delauney_registration_reverse(section_savepath, target_img_path, source_img_path, source_landmark_points0, target_landmark_points0):

    #tempfolder = "C:/Users/q371ms/Downloads/delauney"
    target_img = cv.imread(source_img_path)  # Atlas image   ################
    source_img = cv.imread(target_img_path)  # Section image
    target_gray = cv.cvtColor(target_img, cv.COLOR_BGR2GRAY)
    source_gray = cv.cvtColor(source_img, cv.COLOR_BGR2GRAY)
    mask = np.zeros_like(target_gray)
    height, width, channels = target_img.shape
    ...
    target_nohull = cv.bitwise_and(target_img, target_img, mask=target_hull_mask)
    registered_img = cv.add(target_nohull, target_new_hull)
    #cv.imwrite(os.path.join(section_savepath, 'rev_target_new_hull.png'), target_new_hull)
    return target_new_hull


def func_ardent_registration_reverse(sectionpath, source_img_path, target_img_path):
    target_gr = cv.imread(target_img_path, 0) #section image
    source_rgb = cv.imread(source_img_path) #unlabeled atlas
    b,g,r = cv.split(source_rgb)
    deformed_source_b = transform.transform_image(subject=b,
        subject_resolution=source_resolution,
        output_shape=target_gr.shape, deform_to='template')
    deformed_source_g = transform.transform_image( subject=g,
        subject_resolution=source_resolution,
        output_shape=target_gr.shape, deform_to='template')
    deformed_source_r = transform.transform_image(subject=r,
...
def func_ambia_registration_reverse(reg_code_status, sectionpath):
    ...
    if reg_code_status == "ardent_reg":
        registered_section_img = func_ardent_registration_reverse(sectionpath, source_img_path, target_img_path)
        ...
        ardent_reg_section_img = func_ardent_registration_reverse(sectionpath, source_img_path, target_img_path)
        ardent_reg_section_img_path = os.path.join(sectionpath, "rev_ardent.png")
        registered_section_img = func_delauney_registration_reverse(sectionpath, ardent_reg_section_img_path, target_img_path, source_landmark_points, target_landmark_points)
